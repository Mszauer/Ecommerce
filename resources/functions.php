...
<div class="col-sm-4 col-lg-4 col-md-4">
    <div class="thumbnail">
        <a href="item.php?id={$row['product_id']}"><img style="height:120px;width:120px" src="../resources/{$product_image}" alt=""></a>
        <div class="caption">
            <h6 class="pull-right">&#36;{$row['product_price']}</h6>
    ...
    while($row = fetch_array($query)) {
        ...
        $product = <<<DELIMETER
            <div class="col-md-3 col-sm-6 hero-feature">
                <div class="thumbnail">
                    <img src="../resources/{$product_image}" alt="" style="height:120px;width:120px">
                    <div class="caption">
...
function login_user(){
    if(isset($_POST['submit'])){
        ...
        if(mysqli_num_rows($query) == 0) {
            set_message("<div class=\"alert alert-danger\">Invalid Login Credientials</div>");
            redirect("login.php");
        } else {
...
function send_message() {
    if(isset($_POST['submit'])){ 
        ...
        if(!$result) {
            set_message("<div class=\"alert alert-danger\">Message unable to be sent</div>");
            redirect("contact.php");
        } else {
            set_message("<div class=\"alert alert-success\">Successfully sent message</div>");
            redirect("contact.php");
        }
    }
}
...
function display_orders(){
    ...
    confirm($query);
    while($row = fetch_array($query)) {
        //real paypal: https://www.paypal.com/cgi-bin/webscr?cmd=_view-a-trans&id=XXXXXX
        $orders = <<<DELIMETER
<tr>
    ...
    <td><a href="https://sandbox.paypal.com/cgi-bin/webscr?cmd=_view-a-trans&id={$row['order_transaction']}">View PayPal Details</a></td>
...
function add_product() {
    if(isset($_POST['publish'])) {
        ...
        $last_id = last_id();
        confirm($query);
        set_message("<div class=\"alert alert-success\">Successfully created {$product_title}</div>");
...
function update_product() {
    if(isset($_POST['update'])) {
        ...
        set_message("<div class=\"alert alert-success\">Successfully updated {$product_title}</div>");
        redirect("index.php?products");
        }
}
...
function add_category() {
    if(isset($_POST['add_category'])) {
        ...
        if(empty($cat_title) || $cat_title == " ") {
            echo "<p class='bg-danger'>This field is required</p>";
        } else {
            $insert_cat = query("INSERT INTO categories(cat_title) VALUES('{$cat_title}') ");
            confirm($insert_cat);
            set_message("<div class=\"alert alert-success\">Successfully created category</div>");
        }
    }
}
...
if(isset($_POST['add_user'])) {
    ...
    confirm($query);
    set_message("<div class=\"alert alert-success\">Successfully created user</div>");
...
function add_slides() {
    if(isset($_POST['add_slide'])) {
        ...
        if(empty($slide_title) || empty($slide_image)) {
            set_message("<div class=\"alert alert-danger\">Please upload a slide with an accompanying image</div>");
        } else {
            move_uploaded_file($slide_image_loc, UPLOAD_DIRECTORY . DS . $slide_image);
            $query = query("INSERT INTO slides(slide_title, slide_image) VALUES('{$slide_title}', '{$slide_image}')");
            confirm($query);
            set_message("<div class=\"alert alert-success\">Successfully added slide</div>");
